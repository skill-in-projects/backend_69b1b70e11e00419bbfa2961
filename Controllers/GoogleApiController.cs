...
public class GoogleApiController : ControllerBase
{
    ...
    public async Task<IActionResult> Gemini()
    {
        ...
        try
        {
            var url = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={Uri.EscapeDataString(key)}";
    ...
    /// <summary>GET /api/google/maps - Test Maps Static API (same key works for Maps JavaScript API in frontend).</summary>
    [HttpGet("maps")]
    public async Task<IActionResult> Maps()
    {
        ...
        try
        {
            ...
            var response = await httpClient.GetAsync(url);
            if (response.IsSuccessStatusCode)
                return Ok(new { status = "ok", message = "Maps key valid. Enable Maps Static API and Maps JavaScript API for backend proxy and frontend map display.", service = "Maps" });
            var responseText = await response.Content.ReadAsStringAsync();
